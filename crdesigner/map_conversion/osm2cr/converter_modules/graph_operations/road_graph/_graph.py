...
class Graph:
    ...
    def crop_waypoints_at_intersections(self, intersection_dist: float) -> List[GraphEdge]:
        ...
        for node in self.nodes:
            ...
            # focus on one edge and check its distance to the other edges
                    ...
                    for other_edge in other_edges:
                        ...
                        if config.INTERSECTION_CROPPING_WITH_RESPECT_TO_ROADS:
                            ...
                            while (
                                ...
                                i += 1
                        # check which edge should be deleted.
                        # criterion: edge to delete, where even at the end-point (other node as the currently
                        # ... focused one) the distance to the other edge is not valid (distance to small)
                        ...
                        # get biggest distance for the current edge
                        # distance ... from the current node away along the edge
                        distance = max(
        ...
        for edge, distance, node in to_delete:
            # ################################################################################
            # Artificial change. distance = 0 ... stop cropping edges
            # ################################################################################
            # if config.EXTRACT_SUBLAYER:
            distance = 0
            # [index1:index2] ... interval in which the edge gets kept. Cropping beyond this interval.
