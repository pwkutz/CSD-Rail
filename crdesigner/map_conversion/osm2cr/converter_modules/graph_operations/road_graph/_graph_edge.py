...
class GraphEdge:
    ...
    def get_interpolated_waypoints(self, save=True) -> List[np.ndarray]:
        ...
        if self.interpolated_waypoints is not None:
            ...
            # distance the interpolated points will have from each other (in between start and stop node)
            point_distance = config.INTERPOLATION_DISTANCE_INTERNAL
            d = config.BEZIER_PARAMETER
            result = []
            # 2 given waypoints edge ... start and stop nodes
            if len(self.waypoints) <= 2:
                p1 = self.waypoints[0].get_array()
                p2 = self.waypoints[1].get_array()
                n = max(int(np.linalg.norm(p1 - p2) / point_distance), 2)
                # compute the waypoints (by coordinates) in between the two given waypoints
    ...
    def crop(
        ...
        # cropping: deviding edge by interpolating waypoints. Cut a certain amount of waypoints out.
