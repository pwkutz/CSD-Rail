...
def merge_nodes(
    ...
    # define new created node as GraphNode
    new_node = rg.GraphNode(id_gen.get_id(), central_point[0], central_point[1], edges)
    # create edges for the new created GraphNode
...
def merge_close_intersections(graph: rg.Graph) -> None:
    ...
    while updated:
        for node in graph.nodes:
            # of not has more than 4 edges (incoming, outgoing)
            if node.get_degree() == 4 or True:
                updated = False
                # find all neighbors of the node. Neighbors within certain distance
                nodes_to_merge = collect_neighbors(node, config.MERGE_DISTANCE)
                # ask after nodes with more than one neighbor
                if len(nodes_to_merge) > 1:
                    # delete all nodes and create out of all nodes one node representing all prior nodes.
