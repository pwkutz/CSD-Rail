        ...
        if node.get_degree() == 2:
            ...
            for edge, other in [(edges[0], edges[1]), (edges[1], edges[0])]:
                if edge.node1 == node and edge.oneway:
                    # get incoming and outgoing lanes for an edge referring to a node
                    _, outgoing = lane_linker.get_incomings_outgoings(edge, node)
                    incoming, _ = lane_linker.get_incomings_outgoings(other, node)
                    # edge and lanes straight if incoming and outgoing lanes same length
                    # here: lanes not straight. len(incoming) != len(outgoing)
                    ...
                    # here problem: lanes not straight. len(incoming) != len(outgoing)
