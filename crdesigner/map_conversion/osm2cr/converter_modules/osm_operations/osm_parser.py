    ...
    for node in nodes:
        ...
        if node_id in node_ids:
            ...
            for tag in tags:
                ...
                # CHANGED: define nodes as: pedestrian crossing over railway tracks.

                # if (tag.attrib["k"] == "highway" and tag.attrib["v"] == "crossing"
                # or tag.attrib["k"] == "crossing"):

                # k="railway" v = "crossing" rare. try add. v="railway_crossing", v="level_crossing"
                if (tag.attrib["k"] == "railway" and tag.attrib["v"] == "crossing"
                        or tag.attrib["k"] == "crossing"):
                    crossing_nodes[node_id] = node
    assert len(road_nodes) == len(node_ids)
    # print("Nodes:", len(road_nodes))
    # print("Ways:", len(roads))
...
def get_traffic_signs_and_lights(traffic_rules: Dict) -> (List, List):
    ...
    for rule_id, rule in traffic_rules.items():
        ...
        if ('railway:signal:direction' in rule.keys()
                or 'maxspeed' in rule.keys()
                or 'overtaking' in rule.keys()):
...
def get_ways(accepted_highways: List[str], rejected_tags: Dict[str, str], root) -> OrderedSet[ElTree.Element]:
    ...
    for way in ways:
        ...
        has_maxspeed = False
        roadtype = None
        #a way element ... can possess several tags.
        for tag in tags:
            # Change from motor-highway to railway (rail, tram, subway)
            # if tag.attrib["k"] == "highway" and tag.attrib["v"] in accepted_highways:
            if tag.attrib["k"] == "railway" and tag.attrib["v"] in accepted_highways:
                way.set("roadtype", tag.attrib["v"]) #way ~library
            ...
            if tag.attrib["k"] == "maxspeed":
                has_maxspeed = True
        #CHANGE: config.LOAD_TUNNELS=True. Tunnels get considered. Due to subway.
...
def parse_file(filename: str, accepted_highways: List[str], rejected_tags: Dict[str, str],
    ...
    """
    # put file as element of class-self
    tree = ElTree.parse(filename)
    # #1 element in the file (stored in tree-self)
    root = tree.getroot()
    # Changed!
    # ways ... all railway-<way>'s + library entries for each <way>
    ways = get_ways(accepted_highways, rejected_tags, root)
    # Changed!
    # road_nodes ... nodes with id(node) = ref(nd). Lying on rail-infrastructure
    # crossing_nodes ... road_nodes + displaying additionally crossing position
    ...
    # road_points ... road_nodes on a flat plane. Long-free, lat-free. (OSM = round earth -> long, lat)
    # center_point ... coordinate center in long, lat (according earth curvature)
    # bounds ... [min,max] values earth nodes in long, lat (according to earth curvature)
    road_points, center_point, bounds = get_points(road_nodes, custom_bounds)

    # CHANGED!!!
    # crossing_nodes element road_nodes.
    # crossing_points ... (ID, x, y) - flat plane position (de-earth-curved)
    crossing_points, _, _ = \
        get_points(crossing_nodes, bounds) if len(crossing_nodes) > 0 else (OrderedDict(), None, None)

    # Changed!
    # traffic_rules ... "array" of sign-objects, were each sign-object contains: (key: value, virtual).
    # all nodes / nd (position traffic sign) are stored under their id. traffic_rule[node_id]
    traffic_rules = get_traffic_rules(road_nodes, ways,
                                      config.TRAFFIC_SIGN_KEYS, config.TRAFFIC_SIGN_VALUES)
    # traffic_signs, traffic_lights ... both subsets of traffic_sign. In sum: both subsets build set traffic_sign
    traffic_signs, traffic_lights = get_traffic_signs_and_lights(traffic_rules)
    # find all <relation></relation>-objects with "connectivity" or "restriction" as v-attribute in one of their tags
...
def get_graph_nodes(roads: Set[ElTree.Element], points: Dict[int, Point], traffic_signs: List,
    ...
    # checking: are waypoints elements of several <way></way> element
    # take one accepted <way></way> element
    for road in roads:
        # get all "nd" of the current c
        for waypoint in road.findall("nd"):
            # each nd-element: own ref-number
            point_id = int(waypoint.attrib["ref"])
            if point_id in point_degree:
                # if part of another <way></way> element: +1 to the counter
        ...
        for point_id in (int(road.attrib["from"]), int(road.attrib["to"])):
            ...
            # if not already defined (start + end waypoint of a <way></way> element): define as GraphNodes
    ...
    for traffic_sign in traffic_signs:
        ...
        # Artificially labeled ID with 'road'. NO OSM highway data.
        if point_id.startswith('road'):
            continue
        # if 'road' ... traffic sign already converted to GraphNode.
        # if no 'road'-string in the node ID: define as GraphNode now
    ...
    for traffic_light in traffic_lights:
        ...
        # <node></<node>- and <nd>-element have same id/ref, if at the same location.
        # traffic signs + traffic lights: denoted as <node></<node>-elements
        # <node></<node>-element is OSM-file element. Not the same as a GraphNode. GraphNode is graph element.
        if point_id not in nodes:
            current_point = points[point_id]
            # define as GraphNode
            nodes[point_id] = rg.GraphNode(
    ...
    # defined in the first loop this function
    for point_id in point_degree:
        current_point = points[point_id]
        # ask for intersection waypoints ... waypoints which REF in several ways (<way></way>-element).
        if point_id not in nodes and point_degree[point_id] > 1:
            # define as GraphNodes
            nodes[point_id] = rg.GraphNode(
                point_id, current_point.x, current_point.y, OrderedSet()
            )
    return nodes
...
def get_graph_edges_from_road(roads: Set[ElTree.Element],
    ...
    for road_index, road in enumerate(roads):
        ...
        for index, point in enumerate(point_list):
            ...
            # #####################################################################################
            # FUNCTION WHERE SPLITTING FOR EDGES FOR A NODE (INTERSECTION, TRAFFIC_SIGN) IS DEFINED
            # #####################################################################################
        ...
        for index, waypoint in enumerate(waypoints):
            ...
            # #####################################################################################
            # COMPUTING ANGLES BETWEEN THE DIFFERENT EDGES. IDENTIFYNG SPLITS.
            # #####################################################################################
            ...
            if len(from_edges) == 1:
                ...
                for restriction in restrictions:
                    if restriction.restriction is None:
                        continue
                    # "node", "way" ... two objects (OSM) to which object "relation" can define restrictions.
                    # "restrictions" ... tag-feature of the object "relations" (<relations></relations>)
                    # if neither "node" nor "way" restricted --> error
                    if restriction.via_element_type == "node":
                        if restriction.via_element_id in nodes and nodes[
                            ...
                            continue
                    # if "way" object restricted --> find common node between two edges.
                    # edge --> connection 2 waypoints. Has a defined start and end point.
...
def roads_to_graph(roads: Set[ElTree.Element],
    ...
    # convert different waypoints and nodes to GraphNodes.
    # waypoints of interest: intersection waypoints, waypoints denoting start or stop of way
    # nodes of interest: denoting traffic signs and traffic lights.
    ...
    # edges ... definition split, area of each edge-node in a split case and defining edges between graph-nodes.
    edges = get_graph_edges_from_road(roads, nodes, road_points, bounds, origin)
    # define traffic signs as GraphNodesSign object
    graph_traffic_signs = get_graph_traffic_signs(nodes, edges, traffic_signs)
    # define traffic lights as GraphNodeLight
    graph_traffic_lights = get_graph_traffic_lights(nodes, traffic_lights)
    # considering restrictions (defined in OSM-file) in edge-connections
    ...
    # create graph-object. Assign to the graph-object nodes, edges, ...
...
def create_graph(file_path: str) -> rg.Graph:
    ...
    def _create_graph(
            # roads ... <way></way> elements
            # road_points ... road_nodes on a flat plane. Long-free, lat-free. (OSM = round earth -> long, lat)
            # center_point ... coordinate center in long, lat (according earth curvature)
            # traffic_signs, traffic_lights ... both subsets of traffic_sign.
            # crossing_points ... (ID, x, y) - flat plane position (de-earth-curved)
        ...
        # define graph-object. Use nodes, edges, ...
    ...
    # Personal note... default: config.EXTRACT_SUBLAYER=False
